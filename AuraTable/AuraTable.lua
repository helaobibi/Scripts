...
function AuraTable:OnUpdate(auras)
    ...
    -- Incremental updates should also refresh the cache timestamp
...
function AuraTable:GetUnitBuffs()
    -- 使用 C_UnitAuras.GetAuraDataByIndex 直接遍历
    local i = 1
    while i <= 40 do
        local auraData = C_UnitAuras.GetAuraDataByIndex(self.unit:GetOMToken(), i, 'HELPFUL')
        if not auraData then
            break  -- 没有更多光环了，提前退出
        end
        
        local aura = Bastion.Aura:CreateFromUnitAuraInfo(auraData)
...
function AuraTable:GetUnitDebuffs()
    ...
    local i = 1
    while i <= 40 do
        local auraData = C_UnitAuras.GetAuraDataByIndex(self.unit:GetOMToken(), i, 'HARMFUL')
        if not auraData then
            break  -- 没有更多光环了，提前退出
        end
